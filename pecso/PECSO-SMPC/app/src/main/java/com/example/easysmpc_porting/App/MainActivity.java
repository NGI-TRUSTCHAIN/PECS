...
public class MainActivity extends AppCompatActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private boolean isSMPCEnabled = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Switch activateFLSwitch = findViewById(R.id.switch1);
        activateFLSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                isSMPCEnabled = isChecked;
            }
        });
    ...
    private void getAndPrintAverageSpeed() {
        float averageSpeed = 0;
        String filePath = "/storage/emulated/0/Download";
        try {
            File file = new File(filePath, "engineData.json");
            if (file.exists()) {
                String fileContent = readFile(filePath+"/engineData.json");
                float mySpeed = getSpeedFromEngineData(fileContent);
                Log.d("D", "Speed is "+Float.toString(mySpeed));
                TextView resultView = findViewById(R.id.resultView);
                if (isSMPCEnabled) {
                    EditText serverUrlInput = findViewById(R.id.serverUrlInput);
                    String serverUrl = serverUrlInput.getText().toString();
                    String studyName = "pecsodemo2";
                    String participantName = "client3";
                    String emailReceiving = "pecsosmpc3@libero.it";
                    String passwordReceiving = "megaPECS";

                    // Validate inputs
                    if (serverUrl.isEmpty()) {
                        Toast.makeText(MainActivity.this, "Please type the server URL.", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    dataMap.put("Speed", Float.toString(mySpeed));
                    if (dataMap.isEmpty()) {
                        Toast.makeText(MainActivity.this, "Not enough data to start the secure computation.", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    ...
                    resultView.setText("Securely calculating average speed. Please wait.");

                    // Start EasySMPC
                    startEasySMPC(serverUrl, "participate", studyName, participantName, dataMap,
                            emailReceiving, passwordReceiving);
                } else {
                    averageSpeed = sendToServer(mySpeed);
                    resultView.setText("The average speed of current users is " + averageSpeed + " km/h!");
                }
            } else {
    ...
    private float getSpeedFromEngineData(String jsonString) {
        float speed = 0f;
        try {
